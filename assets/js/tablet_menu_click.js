// MenÃº multinivel por clic para tablets (768px - 1199px)
// Rango: Tablets estÃ¡ndar incluyendo iPad en modo retrato y landscape
// Cubre: iPad Mini (768-1024px), iPad Air (820-1180px), iPad Pro 11" (834-1194px)
// TambiÃ©n cubre Surface Go (768-1366px), Samsung Galaxy Tab (800-1280px), Google Nest Hub (1024-1280px)
document.addEventListener('DOMContentLoaded', function () {
    // Detectar si estÃ¡ en el rango de tablets
    function isTablet() {
    ...
    console.log('Modo tablet detectado - MenÃº por clic activado');

    // Encontrar todos los enlaces que tienen submenÃºs
    // Nivel 2: CategorÃ­as (dentro del ul principal)
    const nivel2Links = document.querySelectorAll('nav[aria-label="MenÃº principal"] > ul > li.relative.group > a');
    // Nivel 3: Alimentos, ElectrodomÃ©sticos, etc (dentro de CategorÃ­as)
    const nivel3Links = document.querySelectorAll('nav[aria-label="MenÃº principal"] > ul > li > ul > li.relative > a');

    console.log('Enlaces nivel 2 encontrados:', nivel2Links.length);
    console.log('Enlaces nivel 3 encontrados:', nivel3Links.length);

    // FunciÃ³n para manejar el "Ver todos los productos"
    ...
    // Manejar clics nivel 2 (CategorÃ­as)
    nivel2Links.forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            if (submenu) {
                ...
                // Cerrar todo
                document.querySelectorAll('nav[aria-label="MenÃº principal"] ul').forEach(ul => {
    ...
    // Prevenir navegaciÃ³n en enlaces que no tienen submenÃºs pero estÃ¡n en el contenedor
    document.querySelectorAll('.nav-links a').forEach(link => {
        if (!link.closest('li:has(ul)')) {
            // Enlaces sin submenÃºs navegan normalmente
            console.log('Enlace sin submenÃº:', link.textContent.trim());
        }
    });
});
